...
if __name__ == '__main__':
    ...
    comprehension_model = pipeline(
        "question-answering", model=COMPREHENSION_MODEL, tokenizer=COMPREHENSION_TOKENIZER, device=use_gpu)

    while True:
        query = input('\nAsk your question: ')
        document_rank_results = rank_with_bert(
            query, model, corpus, embeddings)
        paragraph_rank_results = paragraph_ranking(
            query, model, document_rank_results)
        comprehend_results = comprehend_with_bert(
            comprehension_model, query, paragraph_rank_results)
